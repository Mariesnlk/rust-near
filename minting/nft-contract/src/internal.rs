...
pub(crate) fn refund_deposit(storage_used: u64) {
    ...
    //the attached deposit is greater than or equal to the required cost
...
impl Contract {
    ...
    pub(crate) fn internal_add_token_to_owner(
        ...
        let mut tokens_set = self.tokens_per_owner.get(account_id).unwrap_or_else( || {
           //if the account doesn't have any tokens - create a new unordered set 
           UnorderedSet::new(
    ...
    pub(crate) fn internal_transfer(
        ...
        // If an approval_id is included, check if the sender's actual approval_id is the same as the one included
        ...
        // not send tokens to themselves
        assert_ne!(&token.owner_id, receiver_id,
    ...
    pub(crate) fn internal_remove_token_from_owner(
        ...
        //if the token set is now empty - remove the owner from the tokens_per_owner collection
